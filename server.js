...
// Déclaration de adminLimiter pour sécuriser les routes de génération et d'administration
const adminLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 30, // Limite chaque IP à 30 requêtes par fenêtre pour les actions administratives
    standardHeaders: true,
    legacyHeaders: false,
    handler: (req, res) => {
        securityLog(req, "ADMIN_RATE_LIMIT_EXCEEDED", { ip: req.ip });
        return res.status(429).json({
            success: false,
            error: { code: "TROP_DE_REQUETES_ADMIN", message: "Trop de requêtes administratives. Veuillez patienter avant une nouvelle tentative." }
        });
    }
});
...
// ======================================================
// GENERATION DU SCEAU & KIT DE SÉRIALISATION (Sécurisé par adminLimiter)
// ======================================================
